...
class MCControlAgent(Agent):
	...
	def _run(self, select_action, n_episodes, update, save_interval=None, visualize=False, verbose=False,
	         max_steps=None):
	...
	def test(self, n_episodes, visualize=False, max_steps=None, verbose=False):
		...
		return self._run(select_action, n_episodes, update=False, visualize=visualize, verbose=verbose,
		                 max_steps=max_steps)
	...
	def train(self, n_episodes, verbose=False, save_interval=None, max_steps=None, visualize=False):
		...
		return self._run(select_action, n_episodes, update=True, visualize=visualize, save_interval=save_interval,
	...
	def train(self, n_episodes, visualize=False, verbose=False):
		...
		for i in range(1, n_episodes + 1):
			...
			for i, state in enumerate(states):
				state_action = (state, actions[i])
				if state_action not in visited:
					discount = np.array([self.gamma ** i for i in range(len(states[i:]))])
					state_action_values[state_action][0] += sum(rewards[i:] * discount)
...
class QLearningAgent(Agent):
	...
	def _run(self, select_action, n_episodes, max_steps=None, update=True, visualize=False, verbose=False,
	         save_interval=None):
		def get_epsilon(epi):
	...
	def train(self, n_episodes, save_interval=None, visualize=False, max_steps=None, verbose=False):
		...
		return self._run(select_action, n_episodes, update=True, visualize=visualize, verbose=verbose,
...
class SARSAAgent(Agent):
	...
	def test(self, n_episodes, visualize=False, max_steps=None, verbose=False):
		...
		return self._run(select_action, n_episodes, visualize=visualize, max_steps=max_steps, verbose=verbose,
		                 update=False)
	...
	def train(self, n_episodes, visualize=False, max_steps=None, verbose=False, save_interval=None):
		...
		return self._run(select_action, n_episodes, visualize=visualize, max_steps=max_steps, verbose=verbose,
		                 save_interval=save_interval, update=True)
