...
def run(args):
	...
	agent.compile(
		...
		# p_network=lstm_mlp(32, lstm_shape, [64], activation=tf.nn.leaky_relu, layer_norm=False),
		# v_network=lstm(32, lstm_shape),
		batch_size=(args.nsteps * args.num_envs) // args.nb_batches,
		epochs=args.epochs,
		lr=5e-4,
		ent_coef=0.0,
		vf_coef=.5,
		decay_steps=200,  # 150,  ## args.nb_timesteps / (args.nsteps * args.num_envs)
		max_grad_norm=None,
		shared=False)
...
def parse_args():
	...
	parser.add_argument("--lam", default=.9, action="store_true")
	parser.add_argument("--nb_batches", default=6, action="store_true")
	...
	parser.add_argument("--load_vf", default=False, type=str)
